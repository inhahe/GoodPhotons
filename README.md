...
mesh** — two-sided lambert lighting off **crease-smoothed per-vertex normals** (computed with
the same angle-weighted crease algorithm the renderer's mesh loader uses, at the very crease
angle the object's `smooth=` asked for, so the preview creases where the render will), an
optional depth-tested wireframe overlay, and grey / per-object-tint /
