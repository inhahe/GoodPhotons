...
the 5–50 range usually works). The **~5 noise-/pattern-based** entries (`f_noise3d`,
`f_ridged_mf`, `f_hetero_mf`, `f_ridge`, `f_pattern`) are **not yet** available — they
depend on POV's Perlin-noise and pattern engines (tracked in `known-issues.md`). The
functions are generated by `tools/pov_functions_gen.py` into `src/pov_functions.h` and
evaluated by the same host+device VM, so they render identically on CPU and GPU.
