...
- **`J` — UPBP (mode `D` and mode `M`'s beams, under one weight).** Unifying Points,
  ...
  >
   > **On by default on the CPU; opt-in on the GPU.** `-jsurf` has been the default since 0.260.0
   > (`-nojsurf` is the opt-out, exactly as `-nobeams` is). Since **0.263.0 it runs on the GPU too**,
   > but not by default there: the device estimator still reads ~2.5 % bright against a mode-`R`
   > reference, so it waits behind an explicit `-jsurf`. It used to force the CPU, which was a correctness gate rather than a missing feature:
