...
- **`J` — UPBP (mode `D` and mode `M`'s beams, under one weight).** Unifying Points,
  ...
  >
   > **On by default, and on both backends.** `-jsurf` has been the default since 0.260.0
   > (`-nojsurf` is the opt-out, exactly as `-nobeams` is), and since **0.263.0 it runs on the GPU
   > too**. It used to force the CPU, which was a correctness gate rather than a missing feature:
