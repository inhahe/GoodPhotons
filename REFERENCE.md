...
| `M` | Photon map | Builds a **view-independent** photon map once, then gathers the camera image from it — a direct radius density estimate at the first diffuse hit, or a Jensen final gather one bounce away with `-pmfg <K>` (reusable across cameras). The map is **surfaces only**: add `-beams` for the view-independent photon-beam volume cache, without which participating media render as nothing. Since 0.252.0 a single-camera render **redraws the whole light side each epoch and averages** (photon map, caustic map and beam map, `-beamfreeze` to opt out), so the map's own noise falls with render time instead of freezing on one realization — before that, mode `M` had a floor `-spp` could not touch, most visibly as coloured bars through wavelength-dependent media. (0.253.0 finished the job: a *plain* fixed-`-spp` single-camera render takes a different internal route — the shared/GPU one — which was still freezing.) Multi-camera renders still build the map **once**, which is the amortisation this mode exists for | CPU + **GPU** (both the direct estimate and `-beams`) With `-sunnee` (0.312.0) the sun's **single scatter in media** is marched by next-event estimation along the camera ray instead of gathered from stored beams -- the fix for beam streaks in backlit media; see the flag in the command-line reference. Since 0.317.0 its gather also takes a **layered material's coat lobe** (with probability R, the Fresnel reflectance at that angle) instead of always entering to the body, so a clearcoat shows its sheen and the two backends agree. Since 0.314.0 the gather's **hero wavelength is stratified across each pixel's samples** (jittered strata of the emission CDF under a per-pixel rotation; on the CPU per chunk), which takes the spectral grain of every bright chromatic term -- the sun's disc through a cloud, a rainbow -- from the 1/sqrt(N) of independent draws to ~1/N^1.5; and a camera segment aimed at a `light sun`'s disc takes its media transmittance from 64 ratio-tracking samples instead of one, so the disc holds a steady brightness from frame to frame. |
...
> | **resize the window** | change the preview resolution: the raster renders at the window's **actual pixel dimensions**, so the picture **fills the window (no letterbox bars)** and **shrinking it renders fewer pixels (faster on a heavy scene) while growing it renders more (crisper)**, up to as many pixels as the authored film has, times the display scale squared. On a 150 % display the window's pixels are screen pixels, so it renders up to 2.25× the film's pixels and shows each one on one screen pixel — the old cap at the window's old apparent size, now at full resolution (since 0.367.0; the cap was the film's longest edge, which at the window's opening size, widened by the control strip, made it stretch the render by 7 %). The horizontal field of view widens/narrows with the window (like a game viewport — `fov_y` stays fixed, pixels stay square), so a wider window simply reveals more to the sides |
>
...
- **`-checklayered` (self-test, no scene).** Seven unit tests for the stochastic layered BSDF
  ...
  the closed form `F + (1-F)^2 a / (1 - F a)`; the `f` estimator integrated against cosine must
  reproduce the albedo the sampler produces (this is what pins the refraction Jacobian); `f` must
...
| `layered` | Physical coat over a weighted body: reflect off the coat with prob R, else enter and pick one body lobe (energy-consistent). **CPU + GPU** since 0.317.0. Since 0.323.0 the body is also seen through the coat's **exit** interface: its albedo becomes `a(1-F_dr)/(1-a F_dr)`, exactly 1 at a = 1 (so a white body under a lossless coat conserves energy) and nonlinear below it, so a coated colour **deepens** the way varnish does instead of being washed out by the white specular laid over it. 0.324.0 adds `absorb <spectrum>` (sigma_a, 1/m) + `depth <metres>` for a **tinted lacquer**: Beer-Lambert in, out, and on every internal round trip. `depth` is metres and is NOT `film_thickness`, which is the nanometre wave-optics film | `coat { reflectance fresnel\|thinfilm\|manual, ior, roughness[/roughness_map], film_ior, film_thickness[/film_thickness_map], specular }` + repeated body `layer <material> <weight>` |
...
`power` is exactly one stop brighter). See `scenes/absolute.ftsl`. A rating off by orders of
magnitude — picowatts where watts were meant — quantises the whole image to black with nothing
to say why, while a `-hdr` PFM (written before exposure) looks healthy. Since 0.366.0 ftrace
**warns once per run** when an absolute-exposure image comes out black (its brightest pixel at
or below 8/255), saying how many stops below the auto-exposure level it sits, and the live
preview shows such frames auto-exposed, flagged on its title bar. The written image is unchanged.
...
single self-contained `.glb` needs nothing beside it. Things that happen on the way in and
...
* **A metal is tinted by the base-colour texture, texel by texel** (0.367.2). glTF's metal F0
  *is* its base colour, so a textured metal — a whole-metal material, or the metal lobe of
  `-import-metal mix` — reflects in the texture's colour at every hit, and a vertex colour
  tints it too. Before 0.367.2 a specular material read only its constant reflectance, so
  a textured metal came out untinted: blue sequins rendered white. The raster preview
  matches.
* **The normal map reaches the tracers whatever form the material takes** (0.367.3). A
  dielectric's specular lobe (`-import-specular mix`, the default) and `-import-metal mix`
  both wrap the body in a compound material, and the tracers bend the shading normal once,
  from the material the ray hit, so the map has to sit on the wrapper. From 0.316.0 to
  0.367.2 it sat only on the lobes inside it: **every imported dielectric path-traced as if
  it had no normal map** (the raster preview, which reads the lobes, was unaffected). An
  asset with no `normalTexture` — every glTF in `meshes/` — renders bit-identically.
...
| `-import-specular mix\|layered\|off` | How an imported **glTF dielectric** (`metallic < 0.5`) carries the specular lobe glTF gives it (F0 = `((n-1)/(n+1))^2`, 4 % at the default ior 1.5, modulated by the material's roughness map). **`mix`** (default) is the 0.316.0 form: an uncoloured glossy lobe at constant weight F0 over the body — no angular ramp, but every mode renders it. **`layered`** is the physical coat: a Fresnel interface over the diffuse body, so the lobe also **ramps toward grazing incidence**; `MatType::Layered` runs on the GPU as of 0.317.0 and in BDPT/VCM (`D`, `J`, `U`) as of 0.318.0. **Before 0.367.3, `layered` did nothing:** 0.317.0 documented it as the default, but the importer held the choice in a boolean that folded `layered` into `mix`, so every glTF render since 0.316.0 has used `mix` whatever was asked. The default is now spelled the way it has always behaved, and `layered` works when asked for (making it the default again is a separate, visible change — `known-issues.md`, GLTF-LAYERED-DEFAULT). **`off`** is the pre-0.316.0 flat `diffuse` import. Metals are unaffected by all three. |
| `-import-metal mix\|mean` | How an imported glTF material reads a **metallicRoughness map's metalness**, which glTF defines PER TEXEL while a material here is one BSDF. **`mean`** (default) types the whole material by the map's mean, as every version before 0.339.0 did. **`mix`** honours the map: the body becomes two lobes -- a glossy metal tinted by the base colour (by the base-colour *texture*, per texel, since 0.367.2; before that the metal lobe of a textured material rendered white), and the dielectric -- chosen per texel by the metalness. Under the default `-import-specular mix` the pair takes the place of the dielectric's 4 % lobe (a two-child mix has one weight slot, and metal-vs-dielectric is the bigger error); under `-import-specular layered` it sits inside the coat. Either way it stays one level of compound and still runs on the GPU. It is opt-in because the exports it changes are AI-generator assets whose metalness is a mid-grey nobody intended physically: Alice's map has a mean of 0.281 and a **maximum of 0.612**, so nothing in it is metal, yet honouring it to the letter makes her dress visibly metallic. When a map is part metal and the flag is off, the loader says so -- once per map -- explains the trade-off, and gives a verdict for that model from a heuristic on the map itself (0.368.1): a **clean** map (under 10 % of its texels between 0.15 and 0.85, the rest clearly metal or clearly not -- an intentional mask such as sequins or trim) means you probably want `mix`; a **murky** one (30 % or more -- generator noise) means the default is probably right; anything between says to compare both. The verdict only advises: the default does not change. With the flag on, the same verdict is printed beside the confirmation |
| `-sunnee` (alias `-sun-nee`) | Mode `M` with `-beams`: estimate the sun's **single-scatter** (order-1) contribution in every medium by **next-event estimation along the camera ray** instead of gathering it from stored photon beams. The direct-sun chords (order 1, born on a `light sun`, no surface interaction before the chord) are **erased from the beam map** at build time so nothing is counted twice; everything else -- bounced order-1 light, env-lit chords, order >= 2 -- stays as beams. This is the fix for **beam streaks / shafts in backlit media**: a collimated light through a thin medium is a handful of long coherent beams per pixel, and no realization count smooths that below the kernel's own bias, while the march is a per-pixel integral with no beams in it (64 jittered steps per medium span, a shadow ray and ratio-tracked transmittance to the sun from each). Colour is folded exactly as the beams' would have been (an achromatic medium takes the sun's CIE integral, a `phase rainbow` medium its bow table), so the rain's rainbow is deterministic in colour. **Refuses a `-loadmap` file traced before 0.312.0** (its chords carry no provenance) and says so; retrace the map. Works on both backends. Measured on gallery_rain's backlit frame (320x180, spp 32): energy parity within 1 % per ROI, the frame **faster** (142 s against 163 s -- the erased chords were 44.6 % of the map and every probe walked them), the cloud's structure error 14.05 -> 10.50; combined with the volcache (order >= 2 cached) 76 s and 8.61, and at 128 spp the shafts fanning through the rain are simply gone (`known-issues.md`, "-sunnee"). What remains as beams is the sky-lit order-1 population. |
| `-volcache [res\|off]` | Mode `M` with `-beams` (0.315.0; the FTRACE_VOLCACHE* environment variables it replaces are still honoured): route every **order >= 2** photon-beam chord into a per-medium **spherical-harmonic fluence grid** (`res`^3 per medium, default `48`) at map-build time and erase it from the beam map, so the gather walks only the order-1 remainder and marches the cached field instead -- on both backends. Measured 64 % faster and energy-correct to 0.2-0.4 % on thick media; on gallery_rain the cache holds the multiply-scattered cloud light as a smooth field where the beams painted it as streaks. With `-sunnee` the two together leave only sky-lit order-1 chords as beams. Directional bins (l <= 2) are on for any `g != 0` medium since 0.311.0 -- a scalar cache is 7 % dark on a backlit cloud -- and a `phase rainbow` medium caches through its bow kernel. |
| `-volcache-sh auto\|on\|off` | The cache's angular reconstruction: `auto` (default) uses the l <= 2 bins whenever the medium's `g != 0`, `on` forces them, `off` forces the scalar l = 0 reconstruction (9x smaller, and adequate only for diffusely lit enclosures -- see known-issues, "ANISOTROPIC MEDIA"). |
| `-beams-minorder <n>` | Discard beam chords **below** scattering order `n` at deposit time (default `0` = keep all). Unlike `-beams-order`, which caps further scattering and therefore changes the photon paths, this changes only what is **stored**, so transport is untouched and the omitted energy is simply absent. **Thick media only:** on `_fog_thick` (`sigma_t 20`) order-1 carries 0.4 % of the gathered energy while costing ~17 % of the gather work, and `-beams-minorder 2` measures a **0.24 %** drop in mean radiance; but at `sigma_t 0.6` order-1 carries **50.8 %** and the same flag would halve the image. Check `FTRACE_BEAM_DIAG=1`'s energy line for your scene before using it. |
| `-beams-single` / `-beams-ss` | Shorthand for `-beams-order 1`. |
| `-beamblur <frac>` | Mode-`M` beam kernel half-width, as a fraction of **each medium's own measured mean free path** (default `0.01`). This is the volume estimate's quality knob: larger blurs more beams together (smoother, softer, and slower, since it inflates every sub-beam AABB), smaller keeps the volume crisp at more noise. Per medium, because a scene can hold a dense cloud and a thin rain curtain at once and one radius is simultaneously too blurry for one and too noisy for the other. The mean free path is *measured off the beam map itself* — every stored beam is a sampled free-flight chord, so `mfp = total chord length / chord count` needs no scene access, no wavelength choice, and handles a heterogeneous cloud exactly. **Why the radius is not chosen to hit a sample count (it was, before 0.201.0):** the old rule solved `r = K·A/(2πS)` for a fixed `-beamk`, and total beam length `S` grows with the photon count, so `r ∝ 1/n` — every extra photon bought a *smaller kernel* rather than more samples, and the gather averaged the same `K` monochromatic beams however much you spent. That is a hard colour-noise floor neither `-n` nor `-spp` can move, and it was the cause of the "iridescent" `gallery_rain` cloud. The bias it was buying does not exist either: the `_beams_ms` mode-M-vs-mode-D invariant moves by under one point across a **1000×** radius sweep. **Do not reach for this as a speed knob** — measured on `gallery_rain` at 0.210.0 (one banked `-savemap` re-gathered at four radii through `-loadmap`, so only the kernel moves): gathered beams is **linear** in the radius, not quadratic (8534.6 / 4263.2 / 2125.8 / 1080.1 at `0.01` / `0.005` / `0.0025` / `0.00125`), and gather time falls only *sub*-linearly in that (8× fewer beams bought 4.3× less time) because the split is `-beamsplitmax`-saturated at every radius, so the BVH barely shrinks and you pay near-full traversal for fewer samples. Worse, the noise a small radius adds is **not** `-spp`-reducible: `0.00125` at `-spp 16` finished in *less* wall time than `0.01` at `-spp 4` and was still much grainier (cloud speckle 1.925 vs 1.094), and fitting `speckle² = s₀² + k/spp` gives an **irreducible floor `s₀ ≈ 1.67`**. The beam map is traced once and is view-independent, so extra spp re-gather the *same* beams; the radius fixes how many beams are averaged per gather point, which makes that error a fixed function of the beam set rather than something resampled per sample. Nor is there a free lunch in trading it against `-n`: gathered beams `G` sets *both* the cost (`t ≈ 0.73 + 6.21e-4·G` min/spp on that scene, the beam BVH being pinned identical by `-beamsplitmax`) and the floor (`∝ 1/√G`), so raising `-n` while cutting `-beamblur` to hold `G` constant holds the *cost* constant too — measured at `-n 800M`, `G = 6325.7`, within 1.5% of prediction. What extra `-n` does buy, at equal cost, is **decorrelation**: many more, much shorter, independent chords, which attacks the long *streaks* (neighbouring pixels re-gathering one long chord) rather than the per-gather-point floor. **Per medium (since 0.309.0):** a medium block may carry `beam_blur <frac>` to override this fraction for that medium alone — a soft haze can take a kernel several times wider than a crisp cloud, and the extra gather cost lands only there; see `FTSL.md` §medium. |
 ...
 **Fireflies in media on the GPU? Whether `-beamcount` or `-spp` is the better spend is SCENE-DEPENDENT — measure, do not assume.** Two media scenes, each at matched wall clock, two seeds, scene-linear percentiles: on `_fog_thick` (sigma_t 20) raising `-beamcount` wins, p99 **1.20** against `-spp`'s **3.44**; on `_beams_ms` (sigma_t 6) it is the reverse and by a wider margin, `-beamcount` leaving the tail untouched at p99 **10.56** while `-spp` halves it to **4.26**. Optical depth does not predict which — `_beams_ms` is the thinner medium and has the heavier baseline tail — and neither does beams-gathered-per-probe, which rises comparably on both (7.6x and 16x). The distinction is where the fireflies come from: if the beam estimate is starved, beams help; if the variance is camera-side, only samples do. **Diagnose it** by rendering two seeds at each setting and comparing the p99 of their difference — see `known-issues.md` -> GPU-BEAM-TAIL for the rig. |
| `-beamsplitmax <n>` | Ceiling on **sub-beams after the BVH split** (default `8000000`). It was lowered to `4000000` in 0.290.x on a measured 6.7-15.3 % timing win, then **restored to `8000000` in 0.293.0** when v0.292.x parallelised the BVH build and inverted the trade: the 4 M cap existed only to dodge a build cost that then fell ~3x, and dodging it now costs more traversal than it saves. Paired, same seed: at beamcount 1 M, 8 M wins by 0.42 and 0.59 s; at beamcount 2 M it wins by ~2.7-4.1 s (8-12 %), the gap growing with beam density because a denser beam set makes the cap bind harder. Accuracy is unaffected either way (paired three-seed test, none significant against a 5.3 % seed-to-seed spread). See `known-issues.md` for the full arc. |
...
| `-frames <A> <B>` | With `-camera <pathbase>`: only the path's frames numbered A..B inclusive, by the number in each frame's name (`-camera fly -frames 642 699` renders fly0642..fly0699), so a stopped flyby resumes at the frame it died on and a long run splits across sessions or machines (0.337.0). Selecting nothing is an error naming the path's range |
...
| `-photon-bounce <N>` | Cap the **light** path's bounce count in the host photon pass (modes `M`/`S`, default 32). Distinct from `-max-bounce`, which caps the **camera** path — and in mode `M` the camera path stops at the first diffuse hit, so `-max-bounce` cannot change a mode-`M` image at all (`2`, `32` and `64` are byte-identical) and cannot answer any question about how far light travels before it is deposited. This flag can: `-photon-bounce 2` darkens `fur_creature` by 20 %. **Host only** — the device photon pass has its own cap and ignores this, so pass `-device cpu` when sweeping it. |
| `-gageom <0|1>` | Take the gather's coverage from the **geometric footprint** instead of from probe rays (host only, mode `M`/`S`, off by default). Inert on flat geometry by construction — the footprint is exactly 1.0 there, and `_ga_null` renders byte-identical with it on. Does NOT apply at gather points on a fiber: a coat's footprint measures ~2.9x pi r^2, so dividing by it would make fur about three times darker. Measured against the probe on floor beside a wall, the two agree to within 0.4 %, so the gain (if any) is confined to multi-layer surface geometry rather than simple clipped discs. |
| `-gafparea <r>` / `-gafp-disc <n>` / `-gafp-curve <n>` | Measure the **geometric footprint** — the same-facing surface area actually inside a gather ball of radius `<r>` — and report it per material as a multiple of pi r^2, then exit (no render). That multiple is the factor the photon gather currently assumes is 1. Triangles are sampled by casting `-gafp-disc` stratified rays (default 64) through the tangent disc, so cost per primitive is independent of its size and all layers are counted rather than only the nearest; curve segments are sampled on their own surface at `-gafp-curve` squared points (default 4). Verified: exactly **1.0000** on a flat plane and **0.5986** on floor within 0.25 m of a wall, against an analytic half-disc of 0.5. |
| `-gafootprint <r>` / `-gafootprint-stride <n>` | Census of what is actually inside a gather ball of radius `<r>`, per material, then exit (no render). Fires a primary ray every `<n>`th pixel (default 4), runs `Bvh::traverseSphere` at each hit, and reports the mean number of **curve segments, triangles, spheres, implicits and instances** in the ball. Measured on `fur_creature` at its own adaptive radius: ~2-4 primitives on the unfurred wall and floor against **556-645 curve segments** on the coats — the size of the problem M-GATHERAREA's 8-ray nearest-hit probe is being asked to estimate. Counts are BVH candidates and so a superset of true overlaps (a leaf box can meet the ball when its primitives do not). |
| `-checkspherequery` | Brute-force self-test of the BVH's sphere-overlap traversal (`Bvh::traverseSphere`), then exit. Deterministic and needs no scene: builds 2000 random boxes, fires 500 random query balls (including a degenerate r = 0), and checks the one invariant a footprint measurement depends on — **every primitive whose own box meets the ball is reported**. Exit code is non-zero on any miss. |
| `-roiboxes` | Print a ready-to-use `.rois` file for the selected camera, then exit (no render). Fires one pixel-centre camera ray per pixel, records which material each pixel actually sees, splits each material's mask into connected regions and reports the bounding box of the **largest** one — so a material used in several places does not get a box spanning all of them. Because the boxes come from the renderer's own visibility, at the camera and resolution the render would use, they cannot land on the object next door the way a hand-projected box can. Each line carries `px`, **purity** (fraction of pixels inside the box that really show the material) and **share** (fraction of the material's pixels inside this one region); a box failing either gate is emitted commented-out with the reason, so an untrustworthy ROI can't be copied out by accident. Pair with `-camera <name>` and `-r <w> <h>` to match the renders you intend to score. |
...
| `-radcache-validate <f>` | Fraction of cache reads that trace their tail to full length anyway and report back what the cache *should* have said (default `0.05`). This is what bounds the error — a cell the readers prove wrong is rescaled or retired. `0` disables it and trusts the update pass alone (measured: 4× the energy bias and 2.5× the structural error on a dispersive Cornell). **And `1` disables the cache's *benefit*, which is less obvious than it sounds**: a validating read takes nothing from the cell and traces its tail to full length, so at `1` every read verifies and none terminates — measured, 0 of 1 182 517 consults. The render is still correct, just no faster, and the status line reports `100% of reads verified`. Values near 1 are for auditing a cell's accuracy, never for rendering. |
...
| `-gabias <0\|1>` | **On by default since 0.278.0.** Uses the pseudo-counted coverage `(area+1)/(M+1)` in place of `area/M`. The footprint correction divides by measured coverage, so it evaluates `1/c-hat` of a noisy `c-hat`, and `E[1/c-hat] > 1/E[c-hat]` by Jensen — the correction reads too **bright**, the more so the fewer probes. The pseudo-count removes that: it is exactly 1.0 at full coverage (so flat ground is untouched by construction), is bounded by `M+1`, and is the textbook near-unbiased estimator of `1/p`. Alone it makes total error *worse* (10.8 → 15.0) because it exposes an under-correction the brightening was cancelling; with `-gagate` and `-gaball` the package reaches **8.0**. `-gabias 0` restores `area/M`. |
| `-gagate <n>` | **Default `-1` (no early-out) since 0.278.0.** Holds `-gatherarea`'s flat-interior early-out at a fixed **n** probes instead of `M/4`; `-1` disables it, `0` restores the `M/4` behaviour. The gate returns coverage 1.0 — no correction — when its first `probe0` probes all land on flat-on surface, and `probe0 = M/4` demands 2-of-2 at `M = 8` but 8-of-8 at `M = 32`, so the same disc passes the same test at two different rates purely because the sampling budget changed. Disabling it collapses `cap_gyroid`'s `-gatherarea 8` vs `32` disagreement **17×**. Cost is **scene-dependent, and the earlier "free" claim was measured only where beams dominate**: on `gallery_rain` with `-beams` all arms report 98 % of the camera gather at 1:00, because `-mstats` puts 81 % of that gather in beams — but on the *same scene without* `-beams`, where the surface estimate is 100 % of the gather, disabling the early-out costs **+24 % of surface-gather thread time and ~5 % of frame wall time** (n = 3). Worth it for an unbiased estimator, but not free. |
| `-gaball <0\|1>` | **On by default since 0.278.0.** Accepts a footprint probe only where its hit lies inside the same **ball** the photon query uses. The probe starts `r` above the tangent plane and accepts `h.t <= 2r`, i.e. a **cylinder** of radius `r` and height `2r`, while the estimate's numerator is `queryR(p, r)` — photons within 3D distance `r`. A surface point at tangent offset `rr` and height `dz` sits at `sqrt(rr² + dz²) >= rr`, so on anything non-flat the probe counts rim surface the query can never reach: the divisor comes out too big and the estimate too dark. Worth **+5.5** points on `alice_hair` and **+8.2** on `alice_dress`, while `grid_ground` and `cap_gyroid` — both flat, where `dz = 0` makes the test algebraically inert — read identically at every seed. `-gaball 0` restores the cylinder. |
...
| `-no-spec-nee` / `-spec-nee-hair` | Mode M's gather adds a **glossy** vertex's direct light over the spectral grid rather than at the camera's single sampled wavelength (0.341.0 host, 0.342.0 device) — one shadow ray either way, since a light connection's geometry is wavelength-free. On gallery_rain's gold gyroid it halves the colour noise and moves the mean from 0.79 to 0.91 of a converged mode D, for +16 % (host) / +23 % (device) of the camera pass; `-no-spec-nee` restores the single-wavelength term. `-spec-nee-hair` extends it to **fiber** vertices, and is OFF by default: measured -1.7 % chroma for +26 % cost on Alice, because hair's colour noise is the walk's path variance through the mass, not the connection's wavelength |
...
| Flag | Meaning |
|---|---|
| `-time <s>` | Render until a wall-clock budget. **In mode `M`, `-time`, `-forever` and `-noise` run on the GPU as of 0.295.0.** They drive the device's light-side refresh loop, so each epoch redraws the photon map under a fresh salt at epoch 0's pinned radii and the realizations average -- the map noise falls with the render, not just the gather noise. (`-noise` needs no convergence test: the reported figure is `100/sqrt(spp)`, so `-noise X` is exactly `-spp (100/X)^2`.) `-preview` works there too as of 0.297.0. Lens cameras and multi-camera groups still use the CPU as do lens cameras and any multi-camera group (a flythrough's one shared map is the point; refreshing it would give consecutive frames different realizations rather than convergence). Since 0.290.1 ftrace says so on startup rather than silently falling back. Note the two paths do not mean the same thing by a budget: the progressive driver **rebuilds the photon map under a fresh salt each epoch** and averages the realizations (a `1/sqrt(epochs)` decay), whereas a fixed `-spp` gathers one photon realization, so raising `-spp` averages gather noise but leaves the map's own streaks and blur in place. **Which of `-n` and `-spp` binds is regime-dependent, so measure it rather than guessing.** Seed-to-seed spread at matched spp (two seeds differenced, which cancels everything except what actually varies) gives ratio **1.940** on `_cornell_diffuse` at spp 8 vs 32 — gather noise dominates, so `-spp` converges essentially perfectly — against **1.026** on `_fog_thick`, where the photon realization dominates so completely that quadrupling the samples moves the image 2.6 % and only `-n` converges it (gather noise alone would predict 2.000, a pinned map floor ~1.0). The corollary for the device: on surface scenes its per-sample throughput really is image quality, but in thick media it buys samples that barely change the picture. See `known-issues.md` → `M-TIME-CPU`. |
...
| `-raster` | Fast solid-shaded **preview** (no light transport): z-buffer the whole scene as flat-shaded triangles, one image per selected camera. Honours `-camera` and `-window` (a `camera_curve` flyby animates in the window; a single still becomes an **interactive fly camera** — Space/`+` fly forward, Shift/`-` back, move the mouse off-centre to steer (rate/joystick look, cursor stays visible), wheel = dolly (the step auto-scales to the distance ahead), Ctrl+wheel = bias that step, `C` = wall collision, `0` resets, `P` prints a paste-ready camera, plus **Clip/Reset buttons** in a panel below the image). See the preview note under **Render modes**, and `-explore` below to drop straight into this viewer at a flyby's first frame. **Viewing a bare mesh navigates differently (0.319.0):** when the thing opened is a model file rather than a scene (`ftrace model.glb`), holding the **left mouse button and dragging turns the model** — a turntable about its own centre, which stays framed as it turns. Walking a scene keeps the fly camera unchanged. The two gestures do not collide, so in object view all three are live at once: drag to rotate, Space/Shift to move in and out, and the off-centre hover to steer. Dragging never steers (the hover rate is held at zero while a button is down), which also makes a held button a way to park the cursor anywhere in the image without turning. Implemented as an orbit of the camera, not a transform of the geometry — identical on screen, and free. |
