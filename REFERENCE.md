...
- **`J` — UPBP (mode `D` and mode `M`'s beams, under one weight).** Unifying Points,
  ...
  number of them — see the beam-budget box below), and **`-beamspec` does not apply** while
  **`-beamachro` does** (since 0.250.0). Those are not the same requirement, and treating
  them as one requirement is what left mode `J`'s cloud iridescent until 0.250.0: the
  *bundle* stores wavelengths and no weights, so every wavelength in it must carry identical
  power, and a BDPT light subpath's does not (`Le` is `spd(λ)/pdf(λ)` off the **scene-wide**
  ...
  holds for any sampling density — so mode `J` folds its cloud beams at the emitter's mean
  CIE exactly as mode `M` does. What stays monochromatic is beams in a medium the fold
  refuses, such as a `phase rainbow` rain curtain whose scattering genuinely is chromatic;
  there mode `J` is still chroma-noisier than mode `M`'s `-beamspec 4` at equal `-n`.
...
| `-beamfreeze` | Mode `J`: build the beam map **once** for the whole render, which is how mode `J` behaved before 0.247.0. By default it no longer does, and the reason is worth stating because it changes what "converged" means. Mode `J`'s error has two halves: a camera-side half that `-spp` averages away, and a light-side half **frozen into the beam map** — every camera sample gathers from that same map, so no amount of `-spp` touches it. A frozen mode-`J` render therefore stops at a **noise floor**, and an *invisible* one: the picture goes visually smooth while its overall light level stays several percent off. Measured on `scenes/_fog_thick.ftsl` (`-r 64 -max-bounce 8 -n 256 -beamk 1 -time 60`, whole-frame energy vs a converged mode-`R` reference, eight independent `-seed`s): frozen gives **s.d. 31.6 %**, spanning 0.75–1.63; refreshing gives **s.d. 2.4 %**, spanning 0.96–1.03 — a **13× tighter** result at the same `-n` and the same wall clock, for ~10 % fewer samples. Use `-beamfreeze` to reproduce pre-0.247.0 output (it is bit-identical to it) or to measure what the refresh is worth. It is not otherwise a good idea. |
| `-beamrefresh <frac>` | Share of mode `J`'s wall clock spent rebuilding the light side (default `0.10`). `0` means `-beamfreeze`. The render is split into epochs; between them the light subpaths are re-traced under a fresh salt and the epochs' films are averaged, so the merge noise falls as `1/√epochs` alongside the connection noise instead of standing still. It is affordable because of an asymmetry measured under UPBP-CONV — the *gather* dominates mode `J`, while the whole light side (subpaths, split, BVH) is only ~2.6 % of a render — so decorrelation is nearly free. The epoch length adapts to the **measured** per-epoch overhead rather than a fixed time, which matters because the GPU path re-uploads the scene on every epoch: a heavy scene stretches its epochs out (degrading gracefully toward `-beamfreeze`) while a cheap one refreshes every second or so. Camera samples continue across epoch boundaries via the same absolute-sample-index mechanism `-resume` uses, so nothing is re-drawn and the reported spp is the true total. |
