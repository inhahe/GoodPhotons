...
| `-gabias <0\|1>` | **Diagnostic prototype, off by default.** Replaces `-gatherarea`'s coverage `area/M` with the pseudo-counted `(area+1)/(M+1)`. The footprint correction divides by measured coverage, so it evaluates `1/c-hat` of a noisy `c-hat`, and `E[1/c-hat] > 1/E[c-hat]` by Jensen — the correction reads too **bright**, the more so the fewer probes. The pseudo-count removes that: it is exactly 1.0 at full coverage (so flat ground stays bit-identical), is bounded by `M+1`, and is the textbook near-unbiased estimator of `1/p`. Measured on `gallery_rain` over four seeds, it does what it claims — the `-gatherarea 8` vs `32` disagreement falls from +2.8/+3.1 points to +1.2/−1.4 on hair and cloth — but total error gets **worse** (10.8 → 15.0), because the shipped default's accuracy is partly this same brightening cancelling an under-correction. Off by default for that reason; see M-GATHERAREA in `known-issues.md`. |
| `-gagate <n>` | **Diagnostic prototype, off by default (`0`).** Holds `-gatherarea`'s flat-interior early-out at a fixed **n** probes instead of `M/4`; `-1` disables the early-out entirely. The gate returns coverage 1.0 — no correction — when its first `probe0` probes all land on flat-on surface, and with `probe0 = M/4` that is 2-of-2 at `M = 8` but 8-of-8 at `M = 32`, so the same disc passes the same test at two different rates purely because the sampling budget changed. Measured on `gallery_rain` over four seeds, disabling it collapses `cap_gyroid`'s `-gatherarea 8` vs `32` disagreement **17x** (−7.35 → −0.42 points) and is free (98 % of the camera gather at 1:00 either way). Only together with `-gabias` does the total `M`-dependence actually fall (3.30 → 1.29); either alone merely redistributes it. See M-GATHERAREA in `known-issues.md`. |
