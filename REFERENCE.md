...
| `-import-specular layered\|mix\|off` | How an imported **glTF dielectric** (`metallic < 0.5`) carries the specular lobe glTF gives it (F0 = `((n-1)/(n+1))^2`, 4 % at the default ior 1.5, modulated by the material's roughness map). **`layered`** (default, 0.317.0) is the physical coat: a Fresnel interface over the diffuse body, so the lobe also **ramps toward grazing incidence**. Rendered by modes `M`, `R`, `W` and `A`/`B`/`C` on **both backends** — `MatType::Layered` runs on the GPU as of 0.317.0. **Modes `D`, `J` and `U` refuse layered materials** (a mode gate, not a device one), so for those use **`mix`**: the 0.316.0 form, an uncoloured glossy lobe at constant weight F0 over the body — no angular ramp, but every mode renders it. **`off`** is the pre-0.316.0 flat `diffuse` import. Metals are unaffected by all three. |
