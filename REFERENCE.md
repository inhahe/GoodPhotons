...
- **`J` — UPBP (mode `D` and mode `M`'s beams, under one weight).** Unifying Points,
  ...
  pixel, exactly as in mode `M` (all the `-beam*` knobs apply), and is **on by default** —
  `-nobeams` turns it off, which makes mode `J` mode `D` bit-for-bit. Scope is mode `D`'s
