...
| `-import-specular on\|off` | Whether an imported **glTF dielectric** (`metallic < 0.5`) keeps the specular lobe glTF gives it (default `on`, 0.316.0). The metal-rough model puts a lobe on a dielectric too -- F0 = `((n-1)/(n+1))^2`, 4 % at the default ior 1.5, carrying the **same roughness map** as the metal branch -- and ftrace used to drop it, typing the material a flat `diffuse`: a painted character imported as chalk while the viewer it was exported from showed satin. The lobe is built as a two-child `mix` (an uncoloured glossy lobe at weight F0 over the diffuse body), so every mode and both backends render it. `off` restores the pre-0.316.0 flat import -- the A/B control for the change. Metals are unaffected either way. |
