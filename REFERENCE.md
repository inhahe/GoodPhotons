...
| `-gagate <n>` | **Default `-1` (no early-out) since 0.278.0.** Holds `-gatherarea`'s flat-interior early-out at a fixed **n** probes instead of `M/4`; `-1` disables it, `0` restores the `M/4` behaviour. The gate returns coverage 1.0 — no correction — when its first `probe0` probes all land on flat-on surface, and `probe0 = M/4` demands 2-of-2 at `M = 8` but 8-of-8 at `M = 32`, so the same disc passes the same test at two different rates purely because the sampling budget changed. Disabling it collapses `cap_gyroid`'s `-gatherarea 8` vs `32` disagreement **17×** and is free — 98 % of the camera gather at 1:00 either way, the probes being invisible against the beam gather's 81 % of the frame. |
