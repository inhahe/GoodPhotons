...
| `M` | Many cameras sharing one lighting solution (flythroughs); reusable/persistable map | Fast per frame *(after one shared pass)* | ✓ *(walks to diffuse)* | — | ✓ | ✓ *(direct query)* | Direct query blurs contact shadows (use `-pmfg`); media need `-beams` (single scatter) |
...
Single scattering only, on purpose — the depositing photon crosses straight and is attenuated
by the crossing, so surfaces *behind* the fog are still correctly dimmed, but the
multiple-scatter term is omitted. That is the same trade the `A`/`B` form of `-beams` makes,
and the right one here: multiple scattering in a rain volume is a desaturating grey veil that
washes the bow out.
...
0.9997 deposit-only, 0.9945 on the spectral rainbow). **The one exception is a GRIN scene**: a
photon whose path bends has no straight chord to store, so `-beams` there falls back to the CPU
and prints that it has done so — preferable to the GPU silently dropping the volume, which is
the bug `-beams` exists to fix.
