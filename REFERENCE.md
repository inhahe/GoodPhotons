...
| `-jsurf` / `-jmerge-surf` | **Mode `J` only: turn on the SECOND merge kind — the surface point×point merges mode `U` (VCM) exists to provide.** With it, one mode runs BDPT connections, beam×ray merges in the media *and* vertex merges on the surfaces, all under a single balance heuristic — a strict superset of what mode `D`, mode `J` alone or mode `U` does. The same light subpaths that deposit the beam map also deposit surface photons at every non-delta surface vertex; each camera surface vertex then gathers them within `-jsurf-radius`. Independent of the beams in both directions: a **media-free** scene gets surface merges and no beams (mode `J` says so and carries on), a pure-fog scene beams and no surface photons, and a scene with both gets both under one denominator — which is the point, since a camera path that crosses a cloud and lands on a wall is competed for by all three techniques. **Opt-in for now** (`-nojsurf` is the explicit off) until it is validated against mode `U`; after that it flips to default-on and mode `U` is retired. **Forces the CPU** — a correctness gate, not a gap: the device merge weight carries one merge kind, so a GPU run would both drop the surface merges and *under-weight the beam ones*. See **UPBP-VM** in `known-issues.md`. |
| `-nojsurf` / `-no-jsurf` | Turn mode `J`'s surface point merges back off. Currently the default, so this is only needed to override an earlier `-jsurf` (later flags win) — and to put a mode-`J` render back on the **GPU**, which `-jsurf` forbids. |
