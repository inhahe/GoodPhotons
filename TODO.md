...
- [ ] **Stage 6 — GPU parity** *(bake like `ProcTexture`; split reflect/scalar mirroring 5a)*:
  ...
  - [ ] **6b** — GPU parity for the **SCALAR** (roughness) slot: upload scalar-channel stop programs + driver, device `recSampleScalar` twin, route `dMatRoughness` through it, relax the roughness rejection in `cudaForwardSupported`.
