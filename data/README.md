...
SPDs, 390-700 nm at 10 nm, **visually traced** from the two spectral-power-distribution
plots on p.4 of the GE ConstantColor CMH G12 datasheet (Sep 2013). The plots are
low-resolution raster, so this is a coarse envelope-plus-emission-features trace
(In ~451, Tl ~535, Na-D ~589, rare-earth quasi-continuum); the trace was then
smoothly gain-corrected so its CIE 1931 chromaticity reproduces the datasheet's
published values (3000 K: x=0.435 y=0.400; 4200 K: x=0.379 y=0.374). Generated by
`scraps/cmh_spd.py` (gitignored). Reference blank-continuum xenon/daylight arc
