...
cmh-3000k (`cmh`, warm white), cmh-4200k (`cool-white-mh`): ceramic-metal-halide
SPDs, 390-750 nm at 2 nm, **digitized with `tools/plot_digitizer.py`** (fill-top
mode) from the two spectral-power-distribution plots on p.4 of the GE ConstantColor
CMH G12 datasheet (Sep 2013). The pixel trace follows the plot envelope faithfully,
capturing the characteristic emission lines (In ~451, Tl ~535, Na-D ~589, rare-earth
quasi-continuum). No chromaticity re-tuning is applied: the raw traces land within
~0.01 of the datasheet-rated CIE 1931 xy (3000 K: got 0.4405/0.3888 vs 0.435/0.400;
4200 K: got 0.3702/0.3598 vs 0.379/0.374). Reference blank-continuum xenon/daylight arc
lamps as `preset:bb6000` (a 6000 K Planckian is a faithful visible-range model of a
xenon short-arc, per the Ushio/Osram XBO datasheets).
...
- **DONE (partial): ceramic metal-halide** — `illuminant/cmh-3000k.csv`,
  `cmh-4200k.csv` digitized from the GE ConstantColor CMH G12 datasheet plots (see
  the `illuminant/*.csv` section above). HPS/LPS/mercury still line models.
- **DONE (xenon/arc):** a 6000 K Planckian (`preset:bb6000`) is a faithful
  visible-range model of a xenon short-arc — confirmed against the Osram XBO and
