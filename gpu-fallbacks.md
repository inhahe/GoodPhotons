...
|---|---|---|
| ~~Any `curve` / fiber primitive~~ (`scene.curveSegs` non-empty) | **DONE (0.151.0)** — shipped gated in 0.150.0 with the primitive itself, because a fifth prim range with no device intersector would not *fail*, it would silently trace past every strand and render a furred subject **bald**. Now `DCurveSeg` + `intersectCurveSeg` + a fifth range in `closestHit`/`occluded` (instance indices shifted past it in both). Measured 74× on a 96 000-segment fur patch. **The one non-mechanical part was numerical:** `Real` is fp32 and the round-cone quadric cancels over six decades at fiber scale — a naive port lost 12–36% of hits and erred by 11–42 fiber radii. Fixed by recentering the ray origin to its closest approach before forming the quadric (applied on the host too, so both sides share one formulation), and guarded by `-checkcurve` §6. | ✅ |
...
| ~~`emit pattern:` / `emit_map` emission profile~~ | **DONE (0.82.0)** — shipped CPU-only in 0.80.0 because emission is read from BOTH sides of transport (emission-on-hit *and* the Le at an emitter-sampled point) and MIS combines them, so a partial port biases the image rather than dropping a visible effect. Now `DEmitter::emitPat` / `DMaterial::emitPat` upload, `DEmitTri` carries `uv0/uvE1/uvE2` so a sampled point reports the same (u,v) a hit interpolates, and every device emission read routes through `dEmitPatMul` (hit side) or `dEmitterSamplePointPat` (sampler side). Both `Supported` gates dropped — including the RGB fast path, since an achromatic scalar commutes with the spectral→RGB bake. | ✅ |
| ~~Distant `sun` light (`EmitterShape::Sun`)~~ | **DONE (0.84.0)** — shipped with the CPU feature in the same version. `de.shape == 6` + `DScene::sunCount` upload; `genPhoton`/`genPhotonHero` got the parallel-beam birth branch (cone direction × entry disc of radius `R` perpendicular to it); `dSunSampleCone`/`dInSunCone`/`dSunRadiance` mirror the host; `bkEmitterGeom`/`bkNeeLight`/`bkNeeLightHero`/`bkNeeVolume`/`bkNeeLightRGB` do the cone NEE (`BkNeeGeom` gained a third `sun` kind rather than fusing the product, preserving fp32 rounding for existing scenes); `bkRadiance`/`bkRadianceHero`/`bkRadianceRGB` + the mode-M gather walk + the SPPM camera walk add the direct disc view under the `specularArrival` gate. Validated GPU==CPU 0.01% (mode R), 0.00% (`-rgb`), 0.07% (mode M). | ✅ |
| Any `EmitterShape` with no device branch; an empty-triangle `Mesh` emitter | **safety gate, added 0.99.0.** Every shape maps through one closed whitelist, `deviceEmitterShapeCode()` (just above `cudaForwardSupported`), shared by the gate *and* the `DEmitter` upload so the two cannot drift. It replaces an open ternary chain ending in `: 0` — "anything I don't recognise is a Quad" — which handed the kernel an emitter with a garbage `origin/u/v` basis instead of falling back; malformed geometry inside a kernel can fault the display driver rather than fail cleanly. The whitelist `switch` has **no `default`**, so a newly-added shape trips MSVC C4062 at compile time. Every shape today (`Quad`/`Sphere`/`Spot`/`Env`/`Cylinder`/`Mesh`/`Sun`) *is* implemented, so this rejects nothing right now — it exists to catch the next one added. An empty-triangle `Mesh` emitter is rejected too (the device shape-5 sampler indexes `meshTris` unconditionally, clamping to `-1` on a null pointer). | ✅ gate only |
