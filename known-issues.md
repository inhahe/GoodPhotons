...
### POV-Ray noise-/pattern-based internal functions not ported — 2026-07-13
`src/pov_functions.h` (generated by `tools/pov_functions_gen.py` from POV-Ray's
`source/vm/fnintern.cpp`) ports **73 of POV-Ray's ~79 internal isosurface functions** as
exact algebraic formulas, shared by the CPU (`pattern.h`/`patternEval`) and GPU
(`render_cuda.cu`/`dPatternEval`). **Not yet ported** (the `EXCLUDE` set in the
generator): `f_hetero_mf` (29), `f_ridge` (58), `f_ridged_mf` (59), `f_noise3d` (76),
`f_pattern` (77), `f_noise_generator` (78) — plus the S-table `f_pigment`/`f_transform`/
`f_spline`. These all depend on POV-Ray's engine internals rather than pure algebra:
- **Perlin noise** (`f_noise3d`, `f_noise_generator`, `f_hetero_mf`, `f_ridge`,
  `f_ridged_mf`): need POV's exact `Noise()` — a specific permutation/gradient lattice.
  We have our own value-noise (`patValueNoise`), but it is *not* bit-compatible with
  POV's, so using it would give a different field. Proper fix: port POV's
  `source/core/material/noise.cpp` (hash tables + gradient set) as a host+device
  `povNoise()` and wire it into these functions; then their `.inc` fractal params
  (`Lambda`, `Octaves`, `Omega`, offset, gain) reproduce POV output exactly.
- **Pattern / pigment / transform / spline** (`f_pattern`, `f_pigment`, `f_transform`,
  `f_spline`): reference a whole `TPATTERN`/`PIGMENT`/`TRANSFORM`/`Spline` object via
  `private_data`; they are function-*wrappers* around POV's texturing engine, not
  standalone math. Out of scope until (if ever) that engine is ported. Parser rejects any
  unported name as an "unknown identifier", so scenes fail loudly rather than silently.
