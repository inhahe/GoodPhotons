...
inline int generateCameraSubpath(const Scene& scene, const Camera& cam, const Renderer& mats,
    ...
    path.clear();
    Vertex c;
    c.type = VType::Camera; c.p = cam.eye; c.ns = cam.w; c.ng = cam.w; c.beta = 1.0;
    path.push_back(c);
...
inline double connectBDPT(const Scene& scene, const Camera& cam, const Renderer& mats,
    ...
    if (s == 0) {
        ...
        // Splat a light-subpath vertex onto the camera (light image). s>=2 here
        // (s==1&&t==1 is skipped by the caller), so qs is an interior surface vertex.
