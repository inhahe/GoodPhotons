...
//
// Scope (M1): RECTILINEAR (pinhole) projection and OPAQUE geometry only. Fisheye/
// panoramic projections and the see-through (clear-glass) compositing fall back to the
// CPU. The caller is responsible for that gate; a render() call on an unsupported config
// simply returns an empty vector so the caller can fall back to raster::renderFrame.
//
...
// Render one RECTILINEAR camera to W*H*3 RGB8 (row 0 = image top), matching
// raster::renderFrame's format and exposure. `exposure`, `autoExpose` and `lockAnchor`
// have the same meaning as in raster::renderFrame (the shared host tail applies them).
// Returns an EMPTY vector on any device failure so the caller can fall back to the CPU
// rasterizer. `cam.projection` must be CAM_RECTILINEAR (the caller gates this).
