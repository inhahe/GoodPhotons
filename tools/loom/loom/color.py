"""
Loom colour model — RGB *and* HSV, as first-class **animatable DAG nodes**.
...
- and, because it remembers *how it was authored*, it emits the matching **``.ftsl``
  colour token** (``rgb r g b`` or ``hsv h s v``) so a surface's colour round-trips
  to the renderer in whichever model you wrote it in.

Authoring is either model, interchangeably::
...
loop seam (seamless colour cycling).  ``s``/``v`` are clamped to ``[0, 1]``.

The conversion is done in the graph (:func:`hsv_to_rgb` / :func:`rgb_to_hsv`), so
an HSV colour is a real RGB node downstream — a :class:`Color` *is* its resolved
RGB, while separately remembering its authored channels for token emission.
"""
...
class Color(VecSignal):
    """A 3-component colour authored as RGB or HSV.

    A ``Color`` **is** a :class:`VecSignal` whose components are the resolved
    **RGB** (an HSV colour is converted in the graph), so it works everywhere Loom
    takes a colour.  It also remembers its authored ``mode`` and channels
    (:attr:`src`), so :func:`loom.ftsl_emit.value_token` can emit the matching
    ``.ftsl`` token — ``rgb r g b`` or ``hsv h s v`` — for a surface's colour.
    """

    def __init__(self, components: Vecish, mode: str) -> None:
        if mode not in ("rgb", "hsv"):
            raise ValueError('color mode must be "rgb" or "hsv"')
    ...
    def token(self, clock: Clock, cache: Optional[Cache] = None) -> str:
        """The ``.ftsl`` colour token in the authored model, e.g. ``hsv 0.6 0.8 0.9``."""
