...
class LoopNoise(Signal):
    ...
    def __init__(self, cells: int = 8, seed: int = 0, *,
        ...
        # existing seeds reproduce exactly; gauss uses its own extended key.
        key = f"loomnoise:{int(seed)}:{self.cells}:{self.freq}"
        if dist == "gauss":
            rng = random.Random(f"{key}:gauss:{self.width}")
            lim = self.clip * self.width
            self._vals: List[float] = [max(-lim, min(lim, rng.gauss(0.0, self.width)))
